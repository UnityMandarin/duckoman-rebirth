...
import { isStomp, enemyContact } from './contactRules';
...
export class InteractionSystem {
  ...
  resolvePlayerEnemy(): void {
    ...
    if(result==='dash'){this.enemy.defeat();this.player.chargeUltimate(10);return;}
    if (result==='stomp') {
      this.enemy.defeat();this.player.chargeUltimate(10); this.player.bounceFromStomp(); return;
    }
  ...
  resolveThrownEnemy(): void { if (!this.enemy.defeated && this.throwable.registerEnemyHit()){this.enemy.defeat();this.player.chargeUltimate(10);} }
